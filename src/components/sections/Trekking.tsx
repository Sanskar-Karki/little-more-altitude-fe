                ...
                <div className="absolute top-5 left-5 right-5 z-10 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest backdrop-blur-sm border
                            ${difficulty.includes("Hard") || difficulty.includes("Strenuous")
                        ...
                        >
                            {(difficulty.includes("Hard") || difficulty.includes("Strenuous")) && <Zap size={9} className="inline mr-1 mb-0.5" />}
                            {t(`trekking.filters.${difficulty.toLowerCase()}`)}
                        </span>
                    </div>
                ...
                <div className="absolute inset-x-0 bottom-0 z-10 px-6 pb-6 space-y-3">
                    ...
                    <div className="flex items-center justify-between pt-1">
                        ...
                        <div className="flex items-center gap-2 border border-white/20 group-hover:border-brand-medium group-hover:bg-brand-medium text-white/70 group-hover:text-white pl-4 pr-3 py-2 rounded-full transition-all duration-300">
                            <span className="text-[10px] font-black uppercase tracking-widest">{t('trekking.explore')}</span>
...
export function Trekking({ background = "white", autoFocusSearch = false }: { background?: "white" | "dark"; autoFocusSearch?: boolean }) {
    ...
    const difficulties = [
        { id: "Moderate", label: t('trekking.filters.moderate') },
        { id: "Challenging", label: t('trekking.filters.challenging') },
        { id: "Hard", label: t('trekking.filters.hard') }
    ];

    const durations = [
        { id: "short", label: t('trekking.filters.short') },
        { id: "medium", label: t('trekking.filters.medium') },
        { id: "long", label: t('trekking.filters.long') }
    ];
    ...
    const filteredTreks = useMemo(() => {
        return treks.filter((trek) => {
            ...
            if (selectedDuration) {
                ...
                if (selectedDuration === "short") matchesDuration = days < 10;
                else if (selectedDuration === "medium") matchesDuration = days >= 10 && days <= 15;
                else if (selectedDuration === "long") matchesDuration = days > 15;
            }
                    ...
                    <div className="flex flex-col xl:flex-row gap-4">
                        <div className="relative flex-1 group">
                            ...
                            <input
                                ref={searchInputRef}
                                type="text"
                                placeholder={t('trekking.searchPlaceholder')}
                                value={searchQuery}
                        ...
                        <div className="flex flex-wrap md:flex-nowrap items-center gap-3">
                            <div className={cn("flex flex-wrap md:flex-nowrap items-center gap-1.5 md:gap-2 px-1 py-1 md:px-1.5 bg-white/50 md:bg-white border border-brand-dark/5 md:border-brand-dark/10 rounded-2xl md:rounded-[2rem] shadow-sm", isDark && "bg-brand-white/5 border-white/5")}>
                                <span className={cn("text-[9px] font-black px-3 md:px-4 text-brand-dark/30 uppercase tracking-[0.2em] hidden sm:inline", isDark && "text-white/30")}>{t('trekking.filters.time')}</span>
                            ...
                            <div className={cn("flex flex-wrap md:flex-nowrap items-center gap-1.5 md:gap-2 px-1 py-1 md:px-1.5 bg-white/50 md:bg-white border border-brand-dark/5 md:border-brand-dark/10 rounded-2xl md:rounded-[2rem] shadow-sm", isDark && "bg-brand-white/5 border-white/5")}>
                                <span className={cn("text-[9px] font-black px-3 md:px-4 text-brand-dark/30 uppercase tracking-[0.2em] hidden sm:inline", isDark && "text-white/30")}>{t('trekking.filters.level')}</span>
                                <div className="flex flex-wrap gap-1.5 md:gap-2">
                        ...
                        <div className="flex flex-wrap items-center justify-center lg:justify-start gap-3 mt-6 ml-2">
                            ...
                            {selectedDifficulty && (
                                <div className="flex items-center gap-2 bg-brand-dark text-white px-4 py-2 rounded-full shadow-sm">
                                    <Mountain size={12} />
                                    <span className="text-[11px] font-bold uppercase tracking-widest">{difficulties.find(d => d.id === selectedDifficulty)?.label}</span>
                            ...
                            {selectedDuration && (
                                <div className="flex items-center gap-2 bg-brand-medium text-white px-4 py-2 rounded-full shadow-sm">
                                    <Clock size={12} />
                                    <span className="text-[11px] font-bold uppercase tracking-widest">{durations.find(d => d.id === selectedDuration)?.label.split(" (")[0]}</span>
                        ...
                        {filteredTreks.length === 0 ? (
                            <div className="text-center py-20 md:py-40 bg-brand-dark/[0.02] rounded-[2rem] md:rounded-[4rem] border-2 border-dashed border-brand-dark/10 mt-12 px-6">
                                ...
                                <div className="space-y-4">
                                    <h3 className="text-3xl md:text-5xl lg:text-6xl font-black text-brand-dark tracking-tighter leading-tight">{t('trekking.emptyState.title')}</h3>
                                    <p className="text-brand-dark/40 max-w-sm mx-auto text-base md:text-xl font-medium tracking-tight">
                                        {t('trekking.emptyState.description')}
                                    </p>
                                    <button
                                        onClick={() => {
                                            setSearchQuery("");
                                ...
                                <div className="flex flex-col md:flex-row justify-between items-center md:items-end gap-10">
                                    <div className="flex flex-col gap-4 items-center md:items-start">
                                        <p className={cn("text-[10px] font-black uppercase tracking-[0.2em]", isDark ? "text-white/20" : "text-brand-dark/20")}>
                                            {t('trekking.pagination.showing')} {Math.min(filteredTreks.length, (currentPage - 1) * treksPerPage + 1)}-{Math.min(filteredTreks.length, currentPage * treksPerPage)} {t('trekking.pagination.of')} {filteredTreks.length} {t('trekking.pagination.results')}
                                        </p>
                                        ...
                                        <div className={cn("flex items-center gap-2 px-1.5 py-1 bg-white border border-brand-dark/10 rounded-[2rem] shadow-[0_8px_30px_rgba(0,0,0,0.04)]", isDark && "bg-brand-white/5 border-white/5")}>
                                            <span className={cn("text-[10px] font-black px-4 text-brand-dark/30 uppercase tracking-[0.2em]", isDark && "text-white/30")}>{t('trekking.filters.show')}</span>
